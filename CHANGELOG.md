# Changelog

All notable user-visible changes to WKOpenVR Face Tracking SDK. The "Unreleased" section is auto-appended by `.github/workflows/changelog-append.yml` from conventional-commit subjects on `main`; tagged sections are promoted by `.github/workflows/release.yml` on a `v*` tag push.

The `release.yml` body for each tag is composed mechanically from the commit slice, package artifact hashes, and the templated sections under `.github/release-template/`. Hand-writing release bodies is not part of the workflow.

## Unreleased

### Fixed
- **release:** Tolerate empty beta release notes (4941cad)

---

## v2026.6.16.0-beta -- 2026-06-16

### Fixed
- **release:** Use central date for release automation (ddd8494)

---

## v2026.6.15.0-beta -- 2026-06-15

_Maintenance release; see commit log for details._

---

## v2026.6.14.0-beta -- 2026-06-14

_Maintenance release; see commit log for details._

---

## v2026.6.7.1-beta -- 2026-06-08

### Added
- **sdk:** Add leveled logging interface and module ABI compatibility (7a1b206)

---

## v2026.6.7.0-beta -- 2026-06-08

_Maintenance release; see commit log for details._

---
