...
Download {package-name} from this release and add the folder as a local NuGet source.
"@
    Write-TestFile -Path ".github\release-template\beta.md" -Content @"
## Beta

Beta releases are for module developers testing the next WKOpenVR face-module contract.
"@
    Write-TestFile -Path ".github\release-template\compatibility.md" -Content @"
## Compatibility

Match the SDK package version to the target WKOpenVR host support window.
"@
    Write-TestFile -Path "artifacts\packages\sdk.nupkg" -Content "package"
    ...
    Commit-TestChange -Path "src.txt" -Content "beta`n" -Subject "feat(sdk): add sample contract helper (2026.6.2.0-ABCD)"
    ...
    Assert-Contains -Text $stableNotes -Expected "# WKOpenVR Face Tracking SDK v2026.6.3.0"
    ...
    Assert-Contains -Text $betaNotes -Expected "feat(sdk): add sample contract helper"
